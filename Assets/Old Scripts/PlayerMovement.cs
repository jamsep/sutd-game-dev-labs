...
void Awake()
    ...
    {
        if (next.name == "New Scene")
        {
            // change the position accordingly in your World-1-2 case
            this.transform.position = new Vector3(0f, 0f, 2.0f);
        }
    }
    ...
    public void secret()
    {
        invincibleAudio.PlayOneShot(invincibleAudio.clip);
        inv = !inv;
    }
    public void dead()
    {
        if (!inv) GetComponent<MarioStateController>().SetPowerup(PowerupType.Damage);
    }
